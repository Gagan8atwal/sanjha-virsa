    ...
    <main className="min-h-screen bg-[#fff8ed] text-slate-950">
      <section className="bg-[#24160f] text-white">
        <div className="mx-auto max-w-7xl px-5 py-10 md:px-8">
          <a href="/worlds" className="text-sm font-bold text-amber-300">← Back to Worlds</a>
          <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_0.75fr] lg:items-center">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.35em] text-amber-300">Learning World</p>
              <h1 className="mt-3 text-5xl font-black leading-tight text-white md:text-7xl">{world.title}</h1>
              <p className="mt-3 text-3xl font-black text-amber-200">{world.punjabi}</p>
              <p className="mt-6 max-w-3xl text-lg font-semibold leading-8 text-slate-100">{world.mission}</p>
            </div>
            <div className="rounded-[2rem] bg-white p-6 text-center text-slate-950 shadow-2xl ring-4 ring-amber-300/40">
      ...
      <section className="mx-auto max-w-7xl px-5 py-12 md:px-8">
        <div className="mb-6 flex flex-col justify-between gap-3 md:flex-row md:items-end">
          ...
          <p className="max-w-xl text-slate-700">These cards are the structure for future lessons, audio, quizzes, maps, and story modules.</p>
        </div>

        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {world.cards.map((card) => (
            <a href="#daily-quests" key={card.title} className="group rounded-[2rem] bg-white p-6 shadow-md ring-1 ring-black/10 transition hover:-translate-y-1 hover:shadow-xl">
              ...
              <p className="mt-4 text-sm font-semibold leading-6 text-slate-700">{card.description}</p>
              <div className="mt-5 rounded-2xl bg-amber-300 px-4 py-3 text-center font-black text-slate-950 group-hover:bg-amber-200">{card.action}</div>
            </a>
          ))}
        </div>
      </section>

      <section id="daily-quests" className="bg-white py-12">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-4">
            {world.quests.map((quest, index) => (
              <div key={quest} className="rounded-[2rem] bg-emerald-50 p-5 ring-1 ring-emerald-100">
                ...
                <p className="font-black leading-7 text-slate-950">{quest}</p>
              </div>
            ))}
          </div>
        </div>

const pathways = [
  {
    eyebrow: 'Learn',
    title: 'Punjabi for the next generation',
    description: 'Short language lessons, stories, pronunciation, quizzes, and family practice.',
    href: '/kids',
    accent: 'bg-[#6f1d1b]',
    art: 'language',
  },
  {
    eyebrow: 'History',
    title: 'Understand Punjab through time',
    description: 'Sikh history, Punjab history, places, timelines, and carefully structured learning.',
    href: '/heritage',
    accent: 'bg-[#1e3553]',
    art: 'history',
  },
  {
    eyebrow: 'Culture',
    title: 'See how Punjabi life is lived',
    description: 'Food, clothing, music, village life, festivals, games, arts, and modern Punjabi identity.',
    href: '/culture',
    accent: 'bg-[#315a45]',
    art: 'culture',
  },
    ...
    <main className="sv-page">
      <section className="border-b border-black/10 bg-[#201712] text-white">
        <div className="sv-container grid gap-12 py-14 md:py-20 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:py-24">
          <div>
            ...
            <h1 className="mt-5 max-w-[12ch] font-serif text-5xl font-bold leading-[0.96] tracking-[-0.04em] md:text-7xl">
              A digital home for Punjabi language, history, and culture.
            </h1>
            <p className="mt-6 max-w-2xl text-base font-medium leading-8 text-white/68 md:text-lg">
              Sanjha Virsa helps children and families learn through clear lessons, meaningful stories, respectful history, and interactive cultural exploration.
            </p>
      ...
      <section className="sv-container py-14 md:py-20">
        <div className="max-w-3xl">
          ...
          <p className="sv-copy mt-5">The homepage now stays simple. Each area opens into a focused learning experience instead of placing everything on one crowded page.</p>
        </div>
        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-3">
          {pathways.map((path) => (
            <a key={path.title} href={path.href} className="group overflow-hidden rounded-[1.75rem] border border-black/10 bg-[#fffdf8] shadow-[0_18px_45px_rgba(54,35,24,0.08)] transition hover:-translate-y-1 hover:shadow-[0_24px_60px_rgba(54,35,24,0.13)]">
              <div className={`${path.accent} min-h-56 p-6 text-white`}>
                <PathwayArt type={path.art} />
              </div>
              <div className="p-6">
      ...
      <section className="sv-container py-14 md:py-20">
        <div className="grid gap-6 rounded-[2rem] bg-[#1e3553] p-7 text-white md:p-10 lg:grid-cols-[1fr_auto] lg:items-center">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-[#e7b650]">Built for families worldwide</p>
...
function PunjabLandscape() {
  return (
    <svg viewBox="0 0 640 430" role="img" aria-label="Artistic Punjab landscape with fields, a village home, water, and distant city architecture" className="w-full">
      ...
      <path d="M70 325h160v105H70z" fill="#F3D9A8" />
      <path d="m55 326 95-68 96 68H55Z" fill="#8B3A2B" />
      <rect x="128" y="350" width="46" height="80" rx="4" fill="#6F1D1B" />
      <rect x="88" y="348" width="26" height="34" rx="3" fill="#1E3553" />
      <path d="M400 210h95v82h-95z" fill="#F5E2B7" />
      <path d="M414 210v-36h16v36M465 210v-52h16v52" stroke="#6F1D1B" strokeWidth="10" strokeLinecap="round" />
      <path d="M395 208h105" stroke="#6F1D1B" strokeWidth="8" />
      <path d="M320 430c20-70 52-111 98-155" stroke="#9DC6D8" strokeWidth="25" fill="none" strokeLinecap="round" />
      <path d="M320 430c20-70 52-111 98-155" stroke="#D9EEF4" strokeWidth="8" fill="none" strokeLinecap="round" />
      <g stroke="#F3E4B7" strokeWidth="4" opacity="0.9">
        <path d="M250 335v95M280 327v103M310 328v102" />
        <path d="M520 328v102M550 315v115M580 310v120" />
      </g>
    </svg>
  );
}
...
function PathwayArt({ type }: { type: string }) {
  if (type === 'language') {
    return (
      <svg viewBox="0 0 520 260" role="img" aria-label="Gurmukhi learning illustration" className="h-48 w-full">
        <rect x="28" y="28" width="190" height="190" rx="24" fill="#FFF4DC" opacity="0.96" />
        <text x="123" y="155" textAnchor="middle" fontSize="112" fontWeight="700" fill="#6F1D1B">ਅ</text>
        <path d="M270 70h205M270 118h165M270 166h190" stroke="#F5DFC0" strokeWidth="18" strokeLinecap="round" />
        <path d="M270 214h120" stroke="#E7B650" strokeWidth="12" strokeLinecap="round" />
      </svg>
    );
  }
  if (type === 'history') {
    return (
      <svg viewBox="0 0 520 260" role="img" aria-label="Text-first Sikh and Punjab history illustration" className="h-48 w-full">
        <path d="M75 215h370M105 195h310M135 195V98m83 97V98m84 97V98m83 97V98M112 78h296L260 30 112 78Z" fill="none" stroke="#F4E5C4" strokeWidth="12" strokeLinejoin="round" />
        <circle cx="260" cy="130" r="24" fill="#E7B650" />
        <path d="M260 106v48M236 130h48" stroke="#1E3553" strokeWidth="8" strokeLinecap="round" />
      </svg>
    );
  }
  return (
    <svg viewBox="0 0 520 260" role="img" aria-label="Punjabi culture illustration with phulkari-inspired pattern and village objects" className="h-48 w-full">
      <rect x="40" y="35" width="190" height="190" rx="24" fill="#F7E8C8" />
      <path d="M135 62 182 110 135 158 88 110 135 62Zm0 96 47 48H88l47-48Z" fill="#B24A3A" />
      <path d="M135 86 158 110 135 134 112 110 135 86Z" fill="#D99A22" />
      <path d="M290 190h145v38H290z" fill="#D9A441" />
      <path d="m310 190 52-72 52 72H310Z" fill="#F7E8C8" />
      <rect x="350" y="164" width="24" height="64" fill="#6F1D1B" />
      <path d="M455 214c0-55-20-94-54-124" stroke="#F7E8C8" strokeWidth="14" fill="none" strokeLinecap="round" />
      <path d="M446 92c28 8 38 33 26 57-25-5-38-28-26-57Z" fill="#D99A22" />
    </svg>
  );
}
